...
public class ErstiHelferDAO implements ErstiHelferDAOLocal {
	...
	/*
	 * public int createSession(User user) { ErstiHelferSession newSession = new
	...
	/*
	 * public User findUserByName(String username) { List results =
	 * em.createQuery("SELECT u FROM User u WHERE u.username LIKE :usName")
	 * .setParameter("usName", username).getResultList(); if (results.size() ==
	 * 1) { return (User) results.get(0); } else { return null; } }
	 */

	/*
	 * @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW) public User
	 * createUser(String username, int groupNr) { if (findUserByName(username)
	 * == null) { User user = new User(username, groupNr); em.persist(user);
	 * return user; } else { // Return null, if username already exists. return
	 * null; } }
	 * 
